...
///
#[derive(Debug)]
pub struct HashGrid<F, T> {
    grids: Floors<Grid<u64, Vec<T>>>,
    params: GridParameters<F>,
    bounds: GridBoundary<F>,
    wrap: bool,
}

impl<F, T> HashGrid<F, T>
where
    ...
    T: Clone,
{
    ...
    pub fn new<B>(cells: [u32; 2], floors: u32, bounds: &B, wrap: bool) -> Self
    where
        B: Boundary<F>,
    {
        ...
        let x_cells_f = F::from_u32(cells[0]);
        let y_cells_f = F::from_u32(cells[1]);
        let z_floors_f = F::from_u32(floors);

        // Getting the maximum length on each axis of the grid boundary
        let (length_on_x, length_on_y, length_on_z) = bounds.size().xyz();
        ...
        // initialized as 3D
        let floor_size = (length_on_z / z_floors_f).max(F::one());
        ...
        Self {
            grids: vec![Grid::new(); floors as usize],
            params,
            bounds,
            wrap,
        }
